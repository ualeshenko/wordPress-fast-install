...
setting_bind () {
    ...
    sed -i "s|NS.*.|NS\t${ns1_domain}\.|g" "${path_bind_db}"
    sed -i "s|NS.*.|NS\t${ns2_domain}\.|g" "${path_bind_db}"
...
DNSMGRURL=https://${environment[domain_dnsmgr]}/manager/dnsmgr?out=text&authinfo=${environment[user_dnsmgr]}:${environment[pass_dnsmgr]}
DNSMGRURL=https://${environment[domain_dnsmgr]}/manager/dnsmgr?out=text&authinfo=${environment[user_dnsmgr]}:${environment[pass_dnsmgr]}
CHANGESONLY=yes
...
ns1_domain=ns52.ruweb.net.
path_bind_db="/etc/bind/${base_domain}.db"
