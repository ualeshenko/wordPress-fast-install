...
setting_bind () {
    ...
    echo "zone \"${environment[domain]}\" {
        type master;
        file \"/etc/bind/db.${environment[domain]}\";
};" > /etc/bind/named.conf.local

    cp /etc/bind/db.local "${path_bind_db}"

    sed -i "s/localhost./${environment[domain]}\./g" "${path_bind_db}"
    sed -i "s/127.0.0.1/${environment[ip]}/g" "${path_bind_db}"
    sed -i "s|NS.*.|NS\tns1.${environment[ip]}\.|g" "${path_bind_db}"
    echo -e "ns1\tIN\tA\t${environment[ip]}" >> "${path_bind_db}"
}
...
connect_dnsmanager () {
    ...
    echo -e "NAMEDPATH=/etc/bind/db.${path_bind_db}
MASTERIP=${environment[ip]}
DNSMGRURL=https://dnsmgr1.deserv.net/manager/dnsmgr?out=text&authinfo=${user_dnsmgr}:${password_dnsmgr}
DNSMGRURL=https://dnsmgr1.deserv.net/manager/dnsmgr?out=text&authinfo=${user_dnsmgr}:${password_dnsmgr}
CHANGESONLY=yes
...
\tallow-transfer { 74.119.194.67; 185.12.92.10; }; \ " "${bind_option}" > /tmp/named.conf.options
...
dkim_spf () {
    ...
    echo -e "_domainkey.${domain}.\tTXT\t\"o=~;\"" >> "${path_bind_db}"
    echo -e "mail._domainkey.${domain}.\tTXT\t\"v=DKIM1; k=rsa; p=${public_key}\"" >> "${path_bind_db}"
    echo -e "_adsp._domainkey.adminforum.online.\tTXT\t\"dkim=all\"" >> "${path_bind_db}"
}
...
###System environment
user_dnsmgr="k185.12.94.237"
password_dnsmgr="MVQ690ruDhQNWCS"
path_bind_db="/etc/bind/db.${environment[domain]}"
