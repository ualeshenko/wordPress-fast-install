...
setting_bind () {
    ...
    sed -i "s|NS.*.|NS\tns1.${base_domain}\.|g" "${path_bind_db}"
...
\tnotify explicit; \n\
\talso-notify { '"${ip_domain1_dnsmgr}"'; '"${ip_domain1_dnsmgr}"'; }; \n\
\tallow-transfer { '"${ip_domain1_dnsmgr}"'; '"${ip_domain1_dnsmgr}"'; }; \ ' "${bind_option}" > /tmp/named.conf.options
...
ip_domain1_dnsmgr=$( dig ${environment[domain1_dnsmgr]} +short )
ip_domain2_dnsmgr=$( dig ${environment[domain2_dnsmgr]} +short )
user_dnsmgr="k185.12.94.237"
password_dnsmgr="MVQ690ruDhQNWCS"
path_bind_db="/etc/bind/db.${base_domain}"
